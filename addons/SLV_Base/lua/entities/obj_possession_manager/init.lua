...
function ENT:TargetFaceForward(fcDone)
	if self.entTarget._PossFaceForward then self.entTarget:_PossFaceForward(self.entPossessor, fcDone); return end
...
function ENT:Think()
	...
	if self.nextHealthRegen && CurTime() >= self.nextHealthRegen then
		self.nextHealthRegen = CurTime() +2
	...
	if self.entTarget.bScripted && self.entTarget:GetAIType() != 2 then return end
	local bMove

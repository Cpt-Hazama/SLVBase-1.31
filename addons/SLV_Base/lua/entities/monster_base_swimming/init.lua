	...
	for k, ent in pairs(tblPotentialEnemies) do
		...
		if (bNPC || bPlayer) && !ent:GetNoTarget() then
			...
			if bValid && fDist <= self.fViewDistance && !ent.bSelfDestruct then
				local bCanSee = self:InSight(ent) || ((bIsPlayer && fDist < self.fHearDistance) || (((bIsPlayer && ent:KeyDown(IN_FORWARD) && !ent:Crouching()) || bIsNPC) && fDist <= 100))
				local iDisposition = self:Disposition(ent)
				if bCanSee && self:Visible(ent) && (iDisposition == 1 || iDisposition == 2) && ent:WaterLevel() == 0 then

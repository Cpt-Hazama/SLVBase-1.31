			...
			cam.Start3D(EyePos(), EyeAngles())
				...
				for k, v in pairs(tblNodes) do
					//if tblNodesEnts[k] && EyePos():Distance(tblNodesEnts[k]:GetPos()) <= GetConVarNumber("cl_hlr_nodegraph_visibledist") then
						for _, link in pairs(v.links) do
							local col
							if (iHull == -1 || table.HasValue(math.SplitByPowerOfTwo(link.move),iHull)) && link.dest > k && v.pos:Distance(posShoot) <= flDist && posShoot:Distance(tblNodes[link.dest].pos) <= flDist then //&& (!v.persistent || link.move > 0) then
				...
				if bLinkAValid && ValidEntity(entNodeOver) && entLinkSelectedA != entNodeOver then
					...
					for k, v in pairs(tblNodes[entLinkSelectedA.nodeID].links) do
						if (iHull == -1 || table.HasValue(math.SplitByPowerOfTwo(v.move),iHull)) && v.dest == entNodeOver.nodeID then bLinked = true; break end
					end
					local col
					if !bLinked then
...
for ID, hull in pairs(hulls) do
	...
	local function OnChange(cmd,prev,new)
		new = tonumber(new)
		local tblForceHulls = math.SplitByPowerOfTwo(iForceHullPerm)
		if new == 0 then
			if table.HasValue(tblForceHulls, ID) then
				iForceHullPerm = iForceHullPerm -ID
			end
		elseif !table.HasValue(tblForceHulls, ID) then iForceHullPerm = iForceHullPerm +ID end
	end
	...
	if pl:KeyDown(IN_USE) then
		...
		elseif ValidEntity(entNode) then
			...
			if entNode == entLinkSelectedA then
				...
				if iHull != -1 && !table.HasValue(math.SplitByPowerOfTwo(iForceHullPerm),iHull) then iForceHullPerm = iForceHullPerm +iHull end
				local bLinkExists
				for k, v in pairs(nodegraph.GetNodes(iTypeCur +1)[entLinkSelectedA.nodeID].links) do
					if v.dest == entNode.nodeID then
						bLinkExists = true
						local hullMove = math.SplitByPowerOfTwo(v.move)
						nodegraph.RemoveLink(entLinkSelectedA.nodeID, v.dest, iHull != -1 && table.HasValue(hullMove,iHull) && iHull || nil)
						if iHull != -1 && !table.HasValue(hullMove,iHull) then
							local hullMoveAdd = math.SplitByPowerOfTwo(iForceHullPerm)
							local iForceHullPermAdd = 0
							for k, v in pairs(hullMove) do

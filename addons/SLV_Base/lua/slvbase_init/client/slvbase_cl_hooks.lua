...
usermessage.Hook("ENT_FreezeEffect_Render_End", function(um)
	...
	if !ValidEntity(ent) || !table.HasValue(tblEnts, ent) then return end
...
usermessage.Hook("ENT_FreezeEffect_Render", function(um)
	...
	hook.Add("RenderScreenspaceEffects", "Effect_EntFreezeOverlay", function()
		for k, ent in pairs(tblEnts) do
			if !ValidEntity(ent) then
				tblEnts[k] = nil
				if table.Count(tblEnts) == 0 then hook.Remove("RenderScreenspaceEffects", "Effect_EntFreezeOverlay") return end
			else
				local flBlend = ent:GetNetworkedInt("FreezePercent") *0.01
				render.SetColorModulation(1,1,1)
				cam.Start3D(EyePos(), EyeAngles())
					if util.IsValidModel(ent:GetModel()) || ent:IsPlayer() then
						render.SetBlend(flBlend)
						SetMaterialOverride(mat)
							ent:DrawModel()
							if ent:IsPlayer() then
								/*if ent == LocalPlayer() && GetViewEntity() == ent then
									if flBlend == 1 then
										if !ValidEntity(vmModel) then CreateViewModel() end
										vmModel:DrawModel()
									elseif ValidEntity(vmModel) then vmModel:Remove(); ent:GetViewModel():SetColor(255,255,255,255) end
								end*/
...
function util.ClientsideEffect(...)
	...
	util.Effect(...)
	for k, ent in pairs(ents.FindByClass("class CLuaEffect")) do

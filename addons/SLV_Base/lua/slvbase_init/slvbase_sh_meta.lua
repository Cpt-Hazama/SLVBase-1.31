		...
		timer.Simple(0.5, function() -- Let the entities initialize clientside
			...
			for entA, dat in pairs(tblEntsNoCollide) do
				if !ValidEntity(entA) then tblEntsNoCollide[entA] = nil
					...
					for entB, _ in pairs(dat) do if !ValidEntity(ent) then tblEntsNoCollide[entA][ent] = nil end end
				end
			end
...
function meta:IsPhysicsEntity()
	if !self:IsNPC() && !self:IsPlayer() && !self:IsWorld() && ValidEntity(self:GetPhysicsObject()) then return true end
	return false
end

...
function meta:KnockedDown() return false end
...
function meta:SetFrozen(iPercent)
	...
	if !self.tblFrozen then
		...
		self.tblFrozen.nextPrimary = ValidEntity(wep) && wep:GetNextPrimaryFire() || 0
...
function meta:CanSee(ent, viewAng)
	...
	local ang = self:GetAngleToPos(ent:GetPos())
...
function meta:Ignite(...)
	...
	if GAMEMODE.CanEntityIgnite then
		local bCanIgnite = GAMEMODE:CanEntityIgnite(self,attacker)
		if !bCanIgnite then return end
	end
				...
				for ent, data in pairs(tblEntsIgnited) do
					...
					elseif CurTime() >= data.duration || ent:WaterLevel() == 3 then
						...
						if CurTime() >= data.nextDmg then
							...
							dmg:SetAttacker(ValidEntity(attacker) && attacker || inflictor)
...
function meta:FadeOut(fDelay)
	...
	timer.Create("FadeOutTimer" .. index, 0.01, iLoops, function()
		if !ValidEntity(self) then timer.Destroy("FadeOutTimer" .. index); return end
...
function meta:FadeIn(fDelay)
	...
	timer.Create("FadeInTimer" .. index, 0.01, iLoops, function()
		if !ValidEntity(self) then timer.Destroy("FadeInTimer" .. index); return end
...
function meta:DoExplode(dmg, radius, owner, bDontRemove)
	util.CreateExplosion(self:GetPos(),dmg,radius,self)
